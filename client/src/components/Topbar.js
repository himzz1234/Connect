...
function Topbar() {
  ...
  const fetchUsers = async (username) => {
    const res = await axios.post("http://localhost:8800/api/users", { userId: user?._id });
    setUsers(res.data.filter(user => user.username.toLowerCase().includes(username.toLowerCase())));
  } 

  const followUser = async (id) => {
    await axios.put(`http://localhost:8800/api/users/follow/${id}`, { userId: user?._id })
    window.location.reload();
  }

  const unfollowUser = async (id) => {
    await axios.put(`http://localhost:8800/api/users/unfollow/${id}`, { userId: user?._id })
    window.location.reload();
  }
  ...
  return (
    <div className="fixed top-0 z-20 bg-bodyPrimary w-full px-6">
      <div className="flex items-center">
        ...
        <div className="flex-1 flex relative">
          <div className={`bg-bodySecondary ${users.length > 0 ? 'rounded-t-md' : 'rounded-md'} flex items-center px-3 py-2 w-[500px]`}>
                ...
                <div key={u._id} className={`relative flex space-x-3 items-center ${index > 0 && 'before:absolute before:w-full before:h-[1px] before:bg-[#28343e] before:-top-0'}`}>
                  <div className="flex flex-1 items-center space-x-3 py-3 pl-3">
                    ...
                    <div className="flex-1">
                      <small className="text-[11px] text-[#73899a]">@{u.email?.split('@')[0]}</small>
                      <p className="flex items-center space-x-[0.5px] text-[14px]">
                        {u.username.split('').map((str, index) => {
                          if(debounceSearchTerm.toLowerCase().includes(str.toLowerCase())){
                            return <span key={index} className="font-bold">{str}</span>
                          } else return <span key={index} className='text-[#efefef]'>{str}</span>
                        })}
                      </p>
                    </div>
                  </div>
                  <div>
                    {user?.following.findIndex((f) => f === u._id) > -1 ? <button onClick={() => unfollowUser(u._id)} className="text-[#1da1f2] text-xs px-3 py-1 rounded-sm">Unfollow</button> : <button onClick={() => followUser(u._id)} className="text-[#1da1f2] text-xs px-3 py-1 rounded-sm">Follow</button>}
                  </div>
                </div>
              ))}
            </div>
        ...
        <div onClick={() => setShowDropdown(!showDropdown)} className="cursor-pointer relative ml-14 px-4 py-1.5 rounded-full bg-[hsl(206,23%,16%)] flex items-center">
          <div
